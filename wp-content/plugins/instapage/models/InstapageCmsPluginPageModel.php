...
class InstapageCmsPluginPageModel {
  ...
  public function check($type, $slug = '') {
    ...
    public function display($page, $forcedStatus = null)
    {
        ...
        if (!empty($_GET)) {
            if ($_ENV['PANTHEON_ENVIRONMENT'] && isset($_GET['b64'])) {
        ...
        if (isset($_ENV['PANTHEON_ENVIRONMENT']) && strpos($url, 'PANTHEON_STRIPPED') !== false) {
            ob_start();
            include(__DIR__ . '/../views/landingPageAjaxLoader.php');
            $html = ob_get_clean();
            $result['code'] = 200;
        } else {
