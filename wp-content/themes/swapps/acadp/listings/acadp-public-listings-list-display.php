...
/**
 * This template displays the ACADP listings in list view.
 */
?>
                        ...
                        <ul class="dropdown-menu">
                        	<?php
								...
								foreach( $views as $value => $label ) {
									$active_class = ( 'list' == $value ) ? ' active' : '';							
	...
	<?php while( $acadp_query->have_posts() ) : $acadp_query->the_post(); $post_meta = get_post_meta( $post->ID ); ?>
  		<div <?php the_acadp_listing_entry_class( $post_meta, 'row' ); ?>>
            ...
            <?php if( $can_show_price && isset( $post_meta['price'] ) && $post_meta['price'][0] > 0 ) : ?>
              ...
              ?>
            	<div class="col-md-3 text-right acadp-price-block <?php echo $type_offert;?>">
            		<?php
