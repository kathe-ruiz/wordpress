...
<?php if( !is_user_logged_in() && apply_filters('em_booking_form_show_register_form',true) ): ?>
	...
	<div class="form-group">
		...
		<input class="form-control" type="text" name="user_name" id="user_name" class="input" value="<?php if(!empty($_REQUEST['user_name'])) echo esc_attr($_REQUEST['user_name']); ?>" />
	</div>
	<div class="form-group">
		<label for='dbem_phone'><?php _e('Phone','events-manager') ?></label>
		<input class="form-control" type="text" name="dbem_phone" id="dbem_phone" class="input" value="<?php if(!empty($_REQUEST['dbem_phone'])) echo esc_attr($_REQUEST['dbem_phone']); ?>" />
	</div>
	<div class="form-group">
		<label for='user_email'><?php _e('E-mail','events-manager') ?></label> 
		<input class="form-control" type="text" name="user_email" id="user_email" class="input" value="<?php if(!empty($_REQUEST['user_email'])) echo esc_attr($_REQUEST['user_email']); ?>"  />
	</div>
...
<div class="form-group">
	...
	<textarea class="form-control" style="width: 100%;" name='booking_comment' rows="2" cols="20"><?php echo !empty($_REQUEST['booking_comment']) ? esc_attr($_REQUEST['booking_comment']):'' ?></textarea>
</div>